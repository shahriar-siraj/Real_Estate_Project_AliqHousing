...
var $JssorDebug$ = new function() {
    ...
    $Jssor$ = window.$Jssor$ = new function() {
        ...
        function m(l) {
            if (!j) {
                ...
                if (mb == "Microsoft Internet Explorer" && !!window.attachEvent && !!window.ActiveXObject) {
                    ...
                    j = B;
                    k = h(f.substring(g + 5, f.indexOf(";", g))); /*@cc_on q=@_jscript_version@*/ ;
        ...
        b.$SetStyleClip = function(b, a) {
            if (a) b.style.clip = "rect(" + Math.round(a.$Top || a.$MoveY || 0) + "px " + Math.round(a.$Right) + "px " + Math.round(a.$Bottom) + "px " + Math.round(a.$Left || a.$MoveX || 0) + "px)";
            else if (a !== undefined) {
                var e = b.style.cssText,
                    d = [new RegExp(/[\s]*clip: rect\(.*?\)[;]?/i), new RegExp(/[\s]*cliptop: .*?[;]?/i), new RegExp(/[\s]*clipright: .*?[;]?/i), new RegExp(/[\s]*clipbottom: .*?[;]?/i), new RegExp(/[\s]*clipleft: .*?[;]?/i)],
                    c = t(e, d, "");
            ...
            function a(e, a) {
                ...
                if (c) {
                    ...
                    var s = e.style.filter,
                        u = new RegExp(/[\s]*progid:DXImageTransform\.Microsoft\.Matrix\([^\)]*\)/g),
                        r = t(s, [u], n);
                    K(e, r)
                }
            }
        ...
        b.$FormatEasings = function(a) {
            var b = a || {};
            if (a)
                if ($Jssor$.$IsFunction(a)) b = {
                    $Default: b
                };
                else if ($Jssor$.$IsFunction(a.$Clip)) b.$Clip = {
                $Default: a.$Clip
            };
            return b
        };
...
var $JssorSlideshowFormations$ = window.$JssorSlideshowFormations$ = new function() {
        ...
        f.$FormationSquare = function(h) {
            ...
            for (c = 0; c < a; c++)
                for (d = 0; d < b; d++) e(i, j - Math.min(c + 1 + f, d + 1 + g, a - c + f, b - d + g), [d, c]);
            return i
        };
        f.$FormationRectangle = function(f) {
            ...
            for (a = 0; a < c; a++)
                for (b = 0; b < d; b++) e(g, h - Math.min(a + 1, b + 1, c - a, d - b), [b, a]);
            return g
        };
        f.$FormationRandom = function(c) {
            for (var d = [], a, b = 0; b < c.$Rows; b++)
                for (a = 0; a < c.$Cols; a++) e(d, Math.ceil(1e5 * Math.random()) % 13, [b, a]);
            return d
        };
        f.$FormationCircle = function(c) {
            for (var d = c.$Cols || 1, f = c.$Rows || 1, g = [], a, h = d / 2 - .5, i = f / 2 - .5, b = 0; b < d; b++)
                for (a = 0; a < f; a++) e(g, Math.round(Math.sqrt(Math.pow(b - h, 2) + Math.pow(a - i, 2))), [a, b]);
            return g
        };
        f.$FormationCross = function(c) {
            for (var d = c.$Cols || 1, f = c.$Rows || 1, g = [], a, h = d / 2 - .5, i = f / 2 - .5, b = 0; b < d; b++)
                for (a = 0; a < f; a++) e(g, Math.round(Math.min(Math.abs(b - h), Math.abs(a - i))), [a, b]);
            return g
        };
        f.$FormationRectangleCross = function(f) {
            for (var g = f.$Cols || 1, h = f.$Rows || 1, i = [], a, c = g / 2 - .5, d = h / 2 - .5, j = Math.max(c, d) + 1, b = 0; b < g; b++)
                for (a = 0; a < h; a++) e(i, Math.round(j - Math.max(c - Math.abs(b - c), d - Math.abs(a - d))) - 1, [a, b]);
            return i
        }
    },
    ...
    $JssorSlider$ = window.$JssorSlider$ = function(c, Xb) {
            ...
            function H(l, n, k) {
                if (!F) {
                    ...
                    if (i && k) {
                        ...
                        //$Jssor$.$CssPosition(i, "absolute"); // mnc comment set absolute to slider big image
                    ...
                    if (m) {
                        ...
                        if (e) {
                            var k = I = $Jssor$.$GetNow(),
                                i = o[f(b)];
                            return i.$LoadImage($Jssor$.$CreateCallback(null, R, b, i, e, k), r)
                        }
                    }
                    cb(b)
                } else if (a.$Loop) {
            ...
            function P(a, b, c) {
                ...
                $Jssor$.$Each(d, function(d) {
                    ...
                    if (g == "caption") {
                        ...
                        if (i) {
                            i.border = 0;
                            //$Jssor$.$SetStyles(i, M) //mnc set-image-style
                            $(i).addClass("slider-big-img"); //mnc add class to big image
                        }
                    }
                    P(d, b, c + 1)
                })
        ...
        function zb(d, e) {
            ...
            var g = e ? Q * O() : d,
                f = e ? d : Q * X();
            $Jssor$.$CssWidth(c, g);
            $Jssor$.$CssHeight(c, f);	//mnc resize-set-height
        ...
        var H = !a.$UISearchMode,
            k, g = $Jssor$.$FindChild(c, "slides", H),
        ...
        $Jssor$.$Each(nc, function(a) {
            ...
            else $Jssor$.$IsBrowserIe9Earlier() && $Jssor$.$CssZIndex(a, ($Jssor$.$CssZIndex(a) || 0) + 1)
        });
        var ec = 0,
            e = -1,
            pb, lb, d = K.length,
            ...
            h = Math.min(a.$Cols, d),
            W, j, z, rb, J = [],
            Lb, Nb, Jb, Vb, vc, E, q = a.$PauseOnHover,
...
var $JssorCaptionSlideo$ = window.$JssorCaptionSlideo$ = function(j, c, i) {
    $JssorDebug$.$Execute(function() {
        if (!c.$Transitions) $JssorDebug$.$Error("'$CaptionSlideoOptions' option error, '$CaptionSlideoOptions.$Transitions' not specified.");
        else !$Jssor$.$IsArray(c.$Transitions) && $JssorDebug$.$Error("'$CaptionSlideoOptions' option error, '$CaptionSlideoOptions.$Transitions' is not an array.")
    });
    var a = this,
        k, e = {},
        f = c.$Transitions,
